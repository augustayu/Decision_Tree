...
void create_tree(Node* &root) {
	...
	while(!q.empty()) {
		...
		if (temp->H != 0) {
			...
			// 有可以分割的属性
			if (best_attri != 0) {
				temp->attribute = best_attri; // 记录该节点选用的用于分堆的属性
	...
	while (dataset.getline(buffer, 50)) {
	  if(count > num_of_trainning_set){ 
	   ...
	   // 读取该人各项属性值
		...
		Node* temp = new Node;
		temp = root;
		while( temp->attribute != 0) {
			
			if(p.attributes[temp->attribute] == 'y')
				temp = temp->first_y;
			else if (p.attributes[temp->attribute] == 'n')
				temp =temp->second_n;
			else
