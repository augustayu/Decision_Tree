...
void read_trainning_instance(string name, int num_instance, Node* &root) {
	...
	while (dataset.getline(buffer, 50) ) {
		if(count < num_instance * 87 || count > (num_instance + 1) * 87) {
        for (i = 0; buffer[i] != ','; i++) 
			party_name += buffer[i];
...
double test_accuracy (Node* &root, int num_of_test_subset) {
	ifstream dataset;
	...
	while (dataset.getline(buffer, 50) && count <= 87 * ( num_of_test_subset + 1)) {
	  if(count >= 29 * num_of_test_subset ){ 
		Person p;
        for (i = 0; buffer[i] != ','; i++) 
			party_name += buffer[i];
		p.pre_attribute =( party_name == "republican")? 2:1; // 读取该人党派
